...
function Write-Log {
    ...
    # Write to GUI if available (using BeginInvoke for non-blocking)
    if ($script:WriteGuiLog) {
        try {
            $prefix = switch ($Level) {
                "WARN"  { "⚠ " }
                "ERROR" { "✗ " }
            ...
            # Call WriteGuiLog asynchronously (non-blocking)
            & $script:WriteGuiLog "$prefix$Message"
        }
        catch {
            # GUI update failed, fall back to console
            if (-not $Silent) {
                Write-Host $line
            }
        }
...
function Test-InstallerBusy {
    ...
    try {
        ...
        foreach ($p in $procs) {
            ...
            # Check if CreationDate is valid before using it
            if ($p.CreationDate) {
                try {
                    # CIM already returns DateTime objects, but check for conversion issues
                    $started = $p.CreationDate
                    $age = ($now - $started).TotalMinutes
                    $recent = $age -le $InstallerBusyMinutes
                }
                catch {
                    # Date conversion failed, assume recent to be safe
                    Write-Log "CreationDate conversion failed for msiexec PID $($p.ProcessId)" "WARN"
                    $recent = $true
                }
            }
            else {
                # No creation date available, assume recent to be safe
                $recent = $true
            }
